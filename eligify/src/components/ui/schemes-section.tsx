...
const schemes = [
    {
        ...
        color: "from-white/[0.03] to-white/[0.06]",
...
export function SchemesSection() {
    return (
        <section id="schemes" className="py-24 bg-[#0e0e0e] relative overflow-hidden">
            {/* Grid Background */}
            <div className="absolute inset-0 grid-bg pointer-events-none" />

            {/* Background blobs */}
            <div className="absolute top-1/2 left-1/4 w-96 h-96 bg-white/[0.02] blur-[100px] rounded-full pointer-events-none" />
            <div className="absolute bottom-1/2 right-1/4 w-96 h-96 bg-white/[0.01] blur-[100px] rounded-full pointer-events-none" />
            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                    ...
                    <button className="flex items-center gap-2 px-6 py-3 rounded-full bg-[#1a1a1a] hover:bg-[#252525] text-white border border-[#252525] transition-all font-medium group">
                        View All Schemes
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {schemes.map((scheme, index) => (
                        <motion.div
                            ...
                            className="group relative rounded-2xl bg-[#0a0a0a] border border-[#1a1a1a] hover:border-white/20 overflow-hidden flex flex-col h-full hover:-translate-y-1 transition-transform duration-300"
                        >
                            {/* Gradient Header */}
                            ...
                            <div className="relative p-6 flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                    <div className={cn("p-3 rounded-xl bg-[#111111] border border-[#1a1a1a] shadow-lg text-white", scheme.iconColor)}>
                                        {scheme.icon}
                                    </div>
                                ...
                                <h4 className="text-xl font-bold text-white mb-2 group-hover:text-white/80 transition-colors line-clamp-1">
                                    {scheme.title}
                                </h4>
                                ...
                                <div className="space-y-3 pt-6 border-t border-[#1a1a1a]">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {scheme.benefits.map((benefit, idx) => (
                                            <span key={idx} className="text-xs font-medium text-white/80 bg-white/[0.04] border border-white/10 px-2 py-1 rounded">
                                                {benefit}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="p-4 bg-[#0e0e0e] border-t border-[#1a1a1a] flex items-center justify-between group-hover:bg-[#111111] transition-colors">
                                <span className="text-sm font-medium text-white/60">Check Eligibility</span>
                                <ArrowRight className="w-4 h-4 text-white/60 -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all" />
                            </div>
                        </motion.div>
                    ))}
                </div>
